...
def run_clustering_features(algo, files, save=False):
	...
	if save:
		...
		with open("results/raw_feature/f3/{}.txt".format(name), "w") as f:
...
def run_clustering_pr(algo, files, save=False):
	...
	if save:
		name = "_".join([f[:4] for f in files])
		with open("results/raw_pr/f3/{}.txt".format(name), "w") as f:
...
# f2_e1 = ["scarlatti_all.csv", "mozart_all.csv", "faure_all.csv", "shostakovich_all.csv"]
# f2_e2 = ["byrd_all.csv", "handel_all.csv", "chopin_all.csv", "scriabin_all.csv"]
# f2_e3 = ["bach_all.csv", "beethoven_all.csv", "schubert_all.csv", "debussy_all.csv"]
# f2_e4 = ["buxtehude_all.csv", "haydn_all.csv", "brahms_all.csv", "alkan_all.csv"]
...
# f3_e1 = ["scarlatti_all.csv", "bach_all.csv", "byrd_all.csv", "buxtehude_all.csv"]
# f3_e2 = ["beethoven_all.csv", "handel_all.csv", "mozart_all.csv", "haydn_all.csv"]
# f3_e3 = ["brahms_all.csv", "chopin_all.csv", "schubert_all.csv", "faure_all.csv"]
# f3_e4 = ["scriabin_all.csv", "shostakovich_all.csv", "debussy_all.csv", "alkan_all.csv"]
...
if __name__ == '__main__':
	...
	for a in csv_comps:
		...
		for b in csv_comps:
			try:
				...
				path = "results/dec_features/trail/{}.txt".format(name)
				with open(path, "r") as f:
					tmp = [float(l) for l in f.readlines()]
					if len(tmp) < 72: 
						data.append(tmp)
			except:
				pass
	...
	plt.title("accuracy changes with deep clustering iteration")
	...
	np.savetxt("results/dec_pr/dec_pr_accuracy.txt", data)
	# data = np.around(np.loadtxt("results/dec_features/dec_features_accuracy.txt"), decimals=2)
	print("here 1")
	...
	# ax.set_title("heatmap of pairwise composer clustering acc - dec pianoroll")
	# fig.tight_layout()
	# plt.show()
